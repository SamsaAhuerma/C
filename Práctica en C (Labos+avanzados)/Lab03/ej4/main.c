...
int main(int argc, char *argv[]) {
    ...
    int menorTemp = menor_temp_min(array);
    printf("la menor temperatura minima historica registrada en Cba = %d\n", menorTemp);

    int output1[YEARS];
    maxTemp_poranio(array,output1);

    int output2[YEARS];
    monthMaxPrec(array,output2);

    for (unsigned int i = 0; i < YEARS; i++){
        printf("La mayor temperatura maxima en %u = %d \n", i + 1980, output1[i]);
        printf("Mes de %u en el que se resgistro la mayor cantidad mensual de precipitaciones = %d\n", i + 1980, output2[i]);
    }
